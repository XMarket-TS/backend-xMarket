...
-- Last modification date: 2021-03-20 02:37:36.232
...
ALTER TABLE admin ADD CONSTRAINT admin_person FOREIGN KEY admin_person (person_person_id)
    REFERENCES person (person_id);

-- Reference: administrador_person (table: manager)
ALTER TABLE manager ADD CONSTRAINT administrador_person FOREIGN KEY administrador_person (person_person_id)
    REFERENCES person (person_id);

-- Reference: bank_account_admin (table: bank_account)
ALTER TABLE bank_account ADD CONSTRAINT bank_account_admin FOREIGN KEY bank_account_admin (admin_admin_id)
    REFERENCES admin (admin_id);

-- Reference: bank_account_branch_office (table: bank_account)
ALTER TABLE bank_account ADD CONSTRAINT bank_account_branch_office FOREIGN KEY bank_account_branch_office (branch_office_id_branch_office)
    REFERENCES branch_office (id_branch_office);

-- Reference: bill_card (table: purchase)
ALTER TABLE purchase ADD CONSTRAINT bill_card FOREIGN KEY bill_card (card_id)
    REFERENCES card (card_id);

-- Reference: branch_office_manager (table: branch_office)
ALTER TABLE branch_office ADD CONSTRAINT branch_office_manager FOREIGN KEY branch_office_manager (manager_manager_id)
    REFERENCES manager (manager_id);

-- Reference: media_product_product (table: media_product)
ALTER TABLE media_product ADD CONSTRAINT media_product_product FOREIGN KEY media_product_product (product_product_id)
    REFERENCES product (product_id);

-- Reference: offers_product (table: offers_register)
ALTER TABLE offers_register ADD CONSTRAINT offers_product FOREIGN KEY offers_product (product_product_id)
    REFERENCES product (product_id);

-- Reference: product_branch_office_branch_office (table: product_branch_office)
ALTER TABLE product_branch_office ADD CONSTRAINT product_branch_office_branch_office FOREIGN KEY product_branch_office_branch_office (branch_office_id_branch_office)
    REFERENCES branch_office (id_branch_office);

-- Reference: product_branch_office_product (table: product_branch_office)
ALTER TABLE product_branch_office ADD CONSTRAINT product_branch_office_product FOREIGN KEY product_branch_office_product (product_product_id)
    REFERENCES product (product_id);
...
ALTER TABLE items ADD CONSTRAINT product_order_item FOREIGN KEY product_order_item (product_id)
    REFERENCES product (product_id);

-- Reference: product_provider_product (table: product_provider)
ALTER TABLE product_provider ADD CONSTRAINT product_provider_product FOREIGN KEY product_provider_product (product_product_id)
    REFERENCES product (product_id);

-- Reference: product_provider_provider (table: product_provider)
ALTER TABLE product_provider ADD CONSTRAINT product_provider_provider FOREIGN KEY product_provider_provider (provider_provider_id)
    REFERENCES provider (provider_id);

-- Reference: purchase_items_purchase (table: items)
ALTER TABLE items ADD CONSTRAINT purchase_items_purchase FOREIGN KEY purchase_items_purchase (purchase_bill_id)
    REFERENCES purchase (bill_id);

-- Reference: purchase_user (table: purchase)
ALTER TABLE purchase ADD CONSTRAINT purchase_user FOREIGN KEY purchase_user (user_user_id)
    REFERENCES user (user_id);

-- Reference: stock_product (table: stock)
ALTER TABLE stock ADD CONSTRAINT stock_product FOREIGN KEY stock_product (product_product_id)
    REFERENCES product (product_id);

-- Reference: user_person (table: user)
ALTER TABLE user ADD CONSTRAINT user_person FOREIGN KEY user_person (person_user_id)
    REFERENCES person (person_id);
